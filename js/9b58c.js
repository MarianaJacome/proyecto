...
*/
;(function(a){a.fn.fastClick=function(b){return a(this).each(function(){a.FastButton(a(this)[0],b)})};a.FastButton=function(b,c){var e,d,f=function(){a(b).unbind("touchend");a("body").unbind("touchmove.fastClick")},g=function(b){b.stopPropagation();f();c.call(this,b);"touchend"===b.type&&a.clickbuster.preventGhostClick(e,d)},h=function(a){(10<Math.abs(a.originalEvent.touches[0].clientX-e)||10<Math.abs(a.originalEvent.touches[0].clientY-d))&&f()};a(b).bind({touchstart:function(c){c.stopPropagation();
a(b).bind("touchend",g);a("body").bind("touchmove.fastClick",h);e=c.originalEvent.touches[0].clientX;d=c.originalEvent.touches[0].clientY},click:g})};a.clickbuster={coordinates:[],preventGhostClick:function(b,c){a.clickbuster.coordinates.push(b,c);window.setTimeout(a.clickbuster.pop,2500)},pop:function(){a.clickbuster.coordinates.splice(0,2)},onClick:function(b){var c,e,d;for(d=0;d<a.clickbuster.coordinates.length;d+=2)c=a.clickbuster.coordinates[d],e=a.clickbuster.coordinates[d+1],25>Math.abs(b.clientX-
...
jQuery(function($) {
  ...
  $(document).on('click', '.cookie-bar .btn-close', function(e) {
    e.preventDefault();
  ...
  $(document).on('click', '.btn-back-to-top', function(e) {
	...
	if($('body').is('.logged-in') && _is_dev) {
	    ...
	    $.ajax($('body').data('ajax-url'), {
	      ...
	      complete: function(t) {
					if(t.responseText == "1") {
						$('link[href*="style.css"]').each(function(a, b) {
							$(b).attr('href', $(b).attr('href') + '&v=' + Date.now());
						});
					}
	      }
	...
	$(document).on('click', '.btn-vita', function(e) {
		var _state = $(this).closest('.team-item').attr('data-vita');
		$(this).closest('.team-grid').find('.team-item').each(function(a, b) {
			$(b).attr('data-vita', 'hidden');
		});
		if(_state == 'hidden') $(this).closest('.team-item').attr('data-vita', 'shown');
	});
	...
	$(document).on('click', '.projects-filter button', function(e) {
		$(this).closest('.projects-filter').find('button').removeClass('active');
		$(this).addClass('active');
		$(this).closest('.module').attr('data-status-filter', $(this).val());
		var _basicscroll_fix = window.setInterval(function() { _update_basicscroll_instances(); }, 10);
		window.setTimeout(function() { window.clearInterval(_basicscroll_fix); }, 1000);
	});
	...
	$(document).on('click', '.scroll-list-outer-wrap button', function(e) {
		
		var _module = $(this).closest('.scroll-list-outer-wrap');
		var _list_wrap = _module.find('.scroll-list-wrap');
		var _item_width = _list_wrap.find('li').outerWidth(true);
		var _current_index = _module.data('current-index');
		if(!_current_index) _current_index = 0;
		
		var _new_index = 0;
		
		if($(this).is('.btn-prev')) {
			_item_width = _item_width * -1;
			_new_index = _current_index-1;
		} else {
			_new_index = _current_index+1;
		}
		
		if(_new_index < 0) _new_index = 0;
		if(_new_index >= _list_wrap.find('li').length) _new_index = (_list_wrap.find('li').length-1);
		_module.data('current-index', _new_index);
		
		var _new_scroll_left = 0;
		_list_wrap.find('li').each(function(a, b) {
			if(a < _new_index) _new_scroll_left += $(b).outerWidth(true);
		});
		
		if(_new_scroll_left <= 0) {
			_new_scroll_left = 0;
			_module.attr('data-scroll-position', 'start');
		} else {
			_module.attr('data-scroll-position', 'inbetween');
		}
		
		if(_new_scroll_left >= _list_wrap.get(0).scrollWidth - _list_wrap.get(0).offsetWidth) {
			_new_scroll_left = _list_wrap.get(0).scrollWidth - _list_wrap.get(0).offsetWidth;
			_module.attr('data-scroll-position', 'end');
		}
		
		_module.attr('data-no-auto-scroll', '');
		_list_wrap.stop(true, true);
		
		_list_wrap.animate({
			scrollLeft: _new_scroll_left
		}, 1000, 'easeOutQuart');
		
	});
	
	$(document).on('mouseleave', '.scroll-list-outer-wrap', function(e) {
		if(!Modernizr.touchevents) {
			var _pointer_overlay = $('#pointer-overlay');
			if(_pointer_overlay.length) {
				_pointer_overlay.css('--pointer-opacity', 0);
			}
		}
	});
	
	$(document).on('mousemove', '.scroll-list-outer-wrap button', function(e) {
		if(!Modernizr.touchevents) {
			var _pointer_overlay = $('#pointer-overlay');
			if(_pointer_overlay.length) {
				var _module = $(this).closest('.scroll-list-outer-wrap');
				_pointer_overlay.css('--pointer-opacity', 0);
				if($(this).is('.btn-next') && _module.attr('data-scroll-position') != 'end') _pointer_overlay.css('--pointer-opacity', 1);
				if($(this).is('.btn-prev') && _module.attr('data-scroll-position') != 'start') _pointer_overlay.css('--pointer-opacity', 1);
			}
		}
	});
	...
	var _home_slideshow_interval = false;	
	$(document).on('document:ready:once transition:almostfinished', function() {		
		if($('.module-hero').length) {
			_home_slideshow_interval = window.setInterval(function() {
				var _current = $('.module-hero .background-image.current');
				var _next = _current.next();
				if(!_next.length) _next = $('.module-hero .background-image').first();
				$('.module-hero .background-image').removeClass('current');
				_next.addClass('current');
			}, 6000);
		}		
	});
	$(document).on('document:leave', function() {
		if(_home_slideshow_interval) {
			window.clearInterval(_home_slideshow_interval);
			_home_slideshow_interval = false;
		}
	});
	...
	var _basicscroll_instances = [];
	
	$(document).on('document:ready:once transition:almostfinished', function() {
	
		$('.module-services .row-2, .module-projects .project-article, .module-page-header.has-image figure').each(function(a, b) {
	...
	$(document).on('isonscreen:ready', function() {
		...
		if(Modernizr.objectfit) {
			...
			$('[data-ios]').each(function(a, b) {
				...
				_ios_items.each(function(c, d) {
					...
					var _item_index = $(d).data('ios-item-index');
					if(!_item_index) _item_index = c;
					$(d).css('--ios-item-index', _item_index);
				});
			});
			...
			$('[data-ios]').addClass('is-set-up').onScreen({
				doIn: function() {
					...
					if(!$(this).is('[data-ios-transition-only]')) $(this).addClass('is-on-screen');
				...
				tolerance: $(window).height() / 2,
				toggleClass: '',
			});

			//$('[data-ios]').slice(0,3).addClass('is-on-screen');
			$('[data-ios-transition-only]').addClass('is-on-screen');
	...
	window.setTimeout(function() { $(document).trigger('isonscreen:ready'); }, 250);
		  ...
		  transitions: [{
	        ...
	        $.scrollTo(0, {
	          ...
	          easing: 'easeOutQuart',
				...
				enter: function() { 
					$(document).trigger('isonscreen:ready');
					$(document).trigger('document:enter');
				},
			
				afterEnter: function(data) { 
				
					$('body').attr('class', $(data.next.container).data('body-class'));
					// $('body').css('background-color', $(data.next.container).data('body-background-color'));
